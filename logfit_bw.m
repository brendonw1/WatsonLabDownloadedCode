%% function [slope, intercept,MSE, R, SlopeCI, P, S, yfit] = logfit_bw(x,y,varargin)
...
% 
%   [slope, intercept, MSE, R, S] = logfit(X,Y,graphType), returns the following:
...
% BW                 xR2: The coefficient of determinationx
% BW                 R: correlation coefficient - BW
% BW                 SlopeCI: Confidence interval on slope estimate (using
%                           parametric/t-stats and polyparci from a downloaded toolbox)
% BW                 P: pvalue for null of no correlation - BW
...
% 
%   [graphType, slope, intercept, MSE, R, S] = logfit(X,Y), if you choose
...
% 'color','markercolor','linecolor' etc
% 
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Changed a few i/o's and also a few other things.  Brendon Watson 2015


function [slope, intercept,MSE, R, SlopeCI, P, S, yfit, extra] = logfit_bw(x,y,varargin)
...
k=1;
if isempty(varargin)
    [slope, intercept,MSE, R, SlopeCI, P, S, yfit, extra] = findBestFit(x,y);
    return;
    ...
    switch lower(varargin{1}); % make all lowercase in case someone put in something different.
        ...
        case {'linear','lin'}
            graphType = 'linear';
%         otherwise            
%             [slope, intercept, MSE, R, S, extra] = findBestFit(x,y,varargin{:});
%             return;
    end
...
switch graphType
    ...
    case 'logx'
        ...
        useY=y2fit;
    case 'loglog'
        useX=log10(x2fit); useY=log10(y2fit);
    case 'linear'
        useX=x2fit; useY=y2fit;
end
...
if isempty(x2fit) || isempty(y2fit)
    ...
    slope=NaN; intercept=NaN; MSE = NaN; R= NaN; SlopeCI = NaN; P = NaN;
...
[pf, S] = polyfit(useX,useY,1);
yfit = polyval(pf,ex);
estY=polyval(pf,useX); % the estimate of the 'y' value for each point.
SlopeCI = polyparci(pf,S,0.95);

%% Calculate MSE and xR2x R
% Note that this is done after the data re-scaling is finished.
MSE = mean((useY-estY).^2); % mean squared error.

%     COVyhaty    = cov(estY,y); % = cov(estimated values, experimental values)
%     R        = (COVyhaty(2).^2) ./(var(estY).*var(y));
%     
[R,P] = corrcoef(useX,useY);
R = R(1,2);
P = P(1,2);
...
switch lower(graphType)
    case 'logy'
        yfit=10.^yfit;
        estY=10.^estY; useY=10.^useY;% need to do this for error estimation
    case 'logx'
        ex=10.^ex;
    case 'loglog'
        yfit=10.^yfit;
...
h = plot(x,y,markerType,'markersize',markerSize,'linewidth',2,'color',markerColor,'tag','fitdots');
set(get(get(h,'Annotation'),'LegendInformation'),'IconDisplayStyle','off'); 

%% Plot the approximate line
hold('on'); % in case hold off was on before
plot(ex,yfit,lineStyle,'linewidth',lineWidth,'color',lineColor,'tag','fitline');
...
% outputs shifted.
function [graphType, slope, intercept, MSE, R, S] = findBestFit(x,y,varargin)
...
for ii=1:4
    [a,b,MSE(ii),R(ii)]=logfit_bw(x,y,testList{ii},varargin{:});
end
warning('on')
% [slope, intercept,MSE, R, SlopeCI, P, S]
...
[slope, intercept,MSE, R, SlopeCI, P, S, yfit]=logfit_bw(x,y,graphType,varargin{:});
